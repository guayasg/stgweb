class PropiedadesController < ApplicationController
  ...
  def destroy
    @propiedad.destroy
    respond_to do |format|
      format.html { redirect_to propiedades_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def propiedad_params
      params.require(:propiedad).permit(:codpropiedad, :tcorto, :tlargo, :tcomercial, :componertcorto_id, :componertlargo_id, :componertcomercial_id, :propnumerica)
    end
end
